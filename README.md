...
| GET | /api/v1/documents/:id/download | 下载案件文件（与文档列表同一成员关系，逐请求校验） |
...
- 预置文档补齐：启动时 `SeedService.SeedPresetDocuments` 幂等修复历史文档——记录以 `INSERT ... ON CONFLICT DO NOTHING` 原子去重并无条件重读，文件经临时文件 + 硬链接原子落位（`EnsureFile`），多实例同时启动也只得到一份记录和一个完整文件，已有文件内容绝不改写，竞态落败方重读后继续启动；重复启动不产生重复记录或重复文件。补齐后下载仍只限对应案件成员与管理员。文件缺失时下载返回统一的 JSON 404（`MsgFileNotFound`），而非静态服务器的纯文本 404。
